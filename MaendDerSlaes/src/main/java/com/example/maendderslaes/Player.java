...
public class Player extends Character {
    ...
    public boolean tryToAttack(Character target, String choice) {
        int damage = switch (choice) {
            case "light" -> lightAttack();
            case "medium" -> mediumAttack();
            case "heavy" -> heavyAttack();
            default -> 0;
        };
    ...
    protected int lightAttack() {
        int landAttack = random.nextInt(1, 6); // 80% chance
    ...
    protected int mediumAttack() {
        int landAttack = random.nextInt(1, 11); // 60% chance
    ...
    protected int heavyAttack() {
        int landAttack = random.nextInt(1, 4); // ~33% chance
    ...
    public void setMaxHP(int maxHealth) { this.maxHP = maxHealth; }
    ...
    // Modifiers
    public void addDefence(int defence) { this.defence += defence; }
    public void addStrength(int strength) { this.strength += strength; }
    public void addMaxHealth(int maxHealth) { this.maxHP += maxHealth; }
    public void addSpeed(int speed) { this.speed += speed; }

    public void negateDefence(int defence) { this.defence -= defence; }
    public void negateStrength(int strength) { this.strength -= strength; }
    public void negateMaxHealth(int maxHealth) { this.maxHP -= maxHealth; }
    public void negateSpeed(int speed) { this.speed -= speed; }

    public void negateMoney(int money) { this.money -= money; }
